...
// 		fatalExit(fmt.Sprintf("Failed to load config: %v", err))
// 	}
// 	return msServer.StartServer(cfg), cfg
// }
...
// func reloadServer(app **fiber.App, cfg **msconfig.Config, configFile *string) {
// 	mslogger.LogWarn("Config file changed. Restarting server...")
...
// 	newApp := msServer.StartServer(newCfg)
// 	newAddr := fmt.Sprintf(":%d", newCfg.Server.Port)
